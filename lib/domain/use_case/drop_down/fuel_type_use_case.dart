...
class FuelTypeUseCase {
  final BaseDropDownsRepository _baseDropDownsRepository;

  FuelTypeUseCase(this._baseDropDownsRepository);

  Future<ResponseModel<List<BasicModel>>> call({
    required context,
  }) async {
    ApiResponse apiResponse = await _baseDropDownsRepository.fuelType();
    ...
    if (apiResponse.response != null &&
      ...
      if (baseModel.status == true) {
        List<BasicModel> fuelTypeTypes = [];

        baseModel.data.forEach((object) => fuelTypeTypes.add(BasicModel.fromJson(object)));
